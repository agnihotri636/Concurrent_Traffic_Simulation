...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        if (it->getType() == ObjectType::objectIntersection)
        {
            ...
            shared_ptr<Intersection> intersection = dynamic_pointer_cast<Intersection>(it);
        ...
        else if (it->getType() == ObjectType::objectVehicle)
        {
            ...
            cv::Size carSize(60, 30);
            cv::Rect carRect(pos - cv::Point2d(carSize.width/2, carSize.height/2), carSize);
            cv::rectangle(_images.at(1), carRect, vehicleColor, -1);
            cv::rectangle(_images.at(1), carRect, cv::Scalar(0, 0, 0), 2); // Draw outline

            // Draw the wheels
            int wheelRadius = 12;
            int wheelSpacing = 15;
            cv::Point2d wheel1Pos(pos.x - carSize.width/2 + wheelSpacing, pos.y + carSize.height/2 - wheelRadius - 2);
            cv::Point2d wheel2Pos(pos.x + carSize.width/2 - wheelSpacing, pos.y + carSize.height/2 - wheelRadius - 2);
            cv::circle(_images.at(1), wheel1Pos, wheelRadius, cv::Scalar(0, 0, 0), -1);
            cv::circle(_images.at(1), wheel2Pos, wheelRadius, cv::Scalar(0, 0, 0), -1);
